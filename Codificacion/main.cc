...
int main(){
//INICIO DEL PROGRAMA
ListaPaciente aux3;   //Objetos auxiliares necesarios para que funcione
Historial constante;	
int x=0; //El entero que nos mueve entre las opciones
string aux6; 

aux3.leePacientes();
aux3.leeCitas(); 	 //Cada que inicia, carga los ficheros de datos
...
while(x!=6){ //Menú en bucle   //Aclaración, uso llamadas system("clear") para limpiar pantalla y cin.get() para pausar. 
	system("clear"); 
...
cout<<"3.-Consultar cita medica"<<endl;               //Interfaz que muestra la pantalla
...
cin.clear();
cin.ignore(); //Para que funcione el getline
...
cin>>DNI;
aux5=DNI.substr(3,2);  //Aquí hago una serie de transformaciones que crean un código id.
idhistorial=aux5;
	
if(1!=aux3.anadepaciente(nombre1,apellidos1,DNI,idhistorial)){cout<<"error"<<endl;} //creo el paciente
...
if(aux3.checkDNI(aux7)==1)  //Busco con el dni en la lista de pacientes
	...
	cin>>ayuda;    //Doy la opcion de ir al menú del paciente
	cout<<"\n";
...
{ 
string fecha1;
cout<<"Introduce la fecha asi: XX/XX/XXXX"<<endl; //Formato XX/XX/XXXX
cin>>fecha1;
	
aux3.mostrarcitasdiarias(fecha1); //Muestra las citas que tienen esa fecha
...
if(aux3.checkDNI(aux6)==1)  //compruebo que existe en la base de datos ese dni
								{


while(aux2!=7)   //Segundo Menú
{
	system("clear");
...
{
system("clear");
aux3.mostrartratamientosdepaciente(aux6);  //muestro tratamientos
cin.ignore();
cin.get();
}
if(aux2==3)
...
cin>>elegir;
			...
			if(elegir==3){
				...
				cin.get();
            					...
            					} else { cout<<"Paciente not found\n\n"<<endl; cin.ignore(); //Esto es de la opcion 4
										cin.get();}
}//Fin de bucle de menu paciente
	
}//Fin de bucle de menu principal

constante.escribehistorialTratamientos();       //Guardo historial
